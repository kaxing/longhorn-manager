...
func New(ctx context.Context, cfg *rest.Config, namespace, serverType string) *WebhookServer {
...
func (s *WebhookServer) admissionWebhookListenAndServe() error {
	client, err := client.New(s.context, s.cfg, s.namespace, s.serverType)
	if err != nil {
		return err
	}
...
func (s *WebhookServer) ListenAndServe() error {
	switch serverType := s.serverType; serverType {
	case "admission":
		return s.admissionWebhookListenAndServe()
	case "conversion":
		return s.conversionWebhookListenAndServe()
	default:
		return fmt.Errorf("unexpected webhook server type %v", serverType)
	}
}
...
func (s *WebhookServer) runAdmissionWebhookListenAndServe(client *client.Client, handler http.Handler, validationResources []admission.Resource, mutationResources []admission.Resource) error {
	...
	client.Core.Secret().OnChange(s.context, "secrets", func(key string, secret *corev1.Secret) (*corev1.Secret, error) {
		...
		port := int32(types.DefaultAdmissionWebhookServerPort)
	...
	return server.ListenAndServe(s.context, types.DefaultAdmissionWebhookServerPort, 0, handler, &server.ListenOpts{
...
func (s *WebhookServer) runConversionWebhookListenAndServe(client *client.Client, handler http.Handler, conversionResources []string) error {
	client.Core.Secret().OnChange(s.context, "secrets", func(key string, secret *corev1.Secret) (*corev1.Secret, error) {
		...
		port := int32(types.DefaultConversionWebhookServerPort)
	...
	return server.ListenAndServe(s.context, types.DefaultConversionWebhookServerPort, 0, handler, &server.ListenOpts{
		Secrets:       client.Core.Secret(),
		CertNamespace: s.namespace,
		CertName:      certName,
		CAName:        caName,
