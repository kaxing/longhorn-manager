...
func (n *nodeValidator) Update(request *admission.Request, oldObj runtime.Object, newObj runtime.Object) error {
	...
	return n.validateNodeOnUpdate(oldNode, newNode)
}
...
func (n *nodeValidator) validateNodeOnUpdate(oldNode, newNode *longhorn.Node) error {
	// Only scheduling disabled node can be evicted
	// Can not enable scheduling on an evicting node
	if oldNode.Spec.EvictionRequested && oldNode.Spec.AllowScheduling {
	...
	if newNode.Spec.EngineManagerCPURequest < 0 || newNode.Spec.ReplicaManagerCPURequest < 0 {
		return werror.NewInvalidError(fmt.Sprintf("found invalid EngineManagerCPURequest %v or ReplicaManagerCPURequest %v during node %v update",
			newNode.Spec.EngineManagerCPURequest, newNode.Spec.ReplicaManagerCPURequest, oldNode.Name), "")
	}
