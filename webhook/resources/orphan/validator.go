...
func (o *orphanValidator) Create(request *admission.Request, newObj runtime.Object) error {
	...
	var err error
	switch {
	case orphan.Spec.Type == longhorn.OrphanTypeReplicaDirectory:
...
func (o *orphanValidator) Update(request *admission.Request, oldObj runtime.Object, newObj runtime.Object) error {
	...
	var err error
	switch {
	case newOrphan.Spec.Type == longhorn.OrphanTypeReplicaDirectory:
		err = checkOrphanForReplicaDirectory(newOrphan)
	default:
		return werror.NewInvalidError(fmt.Sprintf("unknown orphan type %v for orphan %v", newOrphan.Spec.Type, newOrphan.Name), "")
	}
	if err != nil {
		return werror.NewInvalidError(fmt.Sprintf("failed to check orphan %v since %v", newOrphan.Name, err), "")
	}
