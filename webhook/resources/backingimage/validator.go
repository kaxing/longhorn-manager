...
func (b *backingImageValidator) Create(request *admission.Request, newObj runtime.Object) error {
	...
	default:
		return werror.NewInvalidError(fmt.Sprintf("unknown backing image source type %v", backingImage.Spec.SourceType), "")
	}

	if _, err := b.ds.GetBackingImage(backingImage.Name); err == nil {
		return werror.NewInvalidError("backing image already exists", "")
	} else if !apierrors.IsNotFound(err) {
		err = errors.Wrapf(err, "failed to check backing image existence before creation")
