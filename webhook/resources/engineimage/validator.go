...
func (e *engineImageValidator) Create(request *admission.Request, newObj runtime.Object) error {
	engineImage := newObj.(*longhorn.EngineImage)
	if engineImage.Spec.Image == "" {
		return werror.NewInvalidError("cannot create engine image with empty image", "")
	}
	return nil
}
...
func (e *engineImageValidator) Delete(request *admission.Request, oldObj runtime.Object) error {
	...
	if engineImage.Status.RefCount != 0 {
		return werror.NewInvalidError("unable to delete the engine image while being used", "")
	}
	return nil
}
