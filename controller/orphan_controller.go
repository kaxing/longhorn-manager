...
func (oc *OrphanController) reconcile(orphanName string) (err error) {
	...
	if !orphan.DeletionTimestamp.IsZero() {
		...
		return oc.deleteOrphanedData(orphan)
	}
...
func (oc *OrphanController) deleteOrphanedData(orphan *longhorn.Orphan) (err error) {
	log := getLoggerForOrphan(oc.logger, orphan)
...
// If the Unavailable condition status is ture, then the deletion of the orphan CRD won't delete the on-disk data
func (oc *OrphanController) updateUnavailableCondition(orphan *longhorn.Orphan) (err error) {
	...
	node, err := oc.ds.GetNodeRO(orphan.Spec.NodeID)
	if err != nil {
		if !apierrors.IsNotFound(err) {
	...
	if err != nil {
		return errors.Wrapf(err, "failed to get node %v", orphan.Spec.NodeID)
	}
	if isUnavailable {
		reason = longhorn.OrphanConditionTypeDataCleanableReasonNodeUnavailable
		return nil
