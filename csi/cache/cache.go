...
func ActivateCacheDevice(volume, devPath string) error {
	...
	if err := activateMetadataCacheDevice(loopDevPath, path.Base(metadataCacheDevPath), cacheDevSize, defaultCacheBlockSize, sectorSize); err != nil {
		return err
	}
	if err := activateDataCacheDevice(loopDevPath, path.Base(dataCacheDevPath), cacheDevSize, defaultCacheBlockSize, sectorSize); err != nil {
		return err
	}
