...
type DiskInfo struct {
	longhorn.DiskSpec
	// To convert status.Condtions' slice to map type, replace longhorn.Distatus with DiskStatus,
	// the UI does not any change.
	DiskStatus
}
...
func toBackupTargetResource(bt *longhorn.BackupTarget) *BackupTarget {
	...
	condition := types.GetCondition(bt.Status.Conditions, longhorn.BackupTargetConditionTypeUnavailable)
